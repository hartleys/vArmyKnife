...
object VcfAnnotateTX {
     ...
     val DEFAULT_MAP_PARAMS : Seq[ParamStr] = Seq[ParamStr](
           ParamStr(id = "walkFcn",synon=Seq(),ty="String",valueString="s",desc="The function to run.",req=true,initParam = true),
   ...
   val MAPFUNCTIONS_USERMANUALBLOCKS : Seq[internalUtils.commandLineUI.UserManualBlock] = Seq[internalUtils.commandLineUI.UserManualBlock](
       internalUtils.commandLineUI.UserManualBlock(title=Some("Variant Map Functions"),
                                                   lines = Seq("Variant Map Functions are a set of tools that perform operations on a variant set "+
                                                               "one variant at a time. Unlike other parameters and operations, map functions are performed in the order "+
                                                               "that they appear in the command line, after all other operations have been carried out (excluding output ops)."+
                                                               ""), level = 2),
       internalUtils.commandLineUI.UserManualBlock(title=Some("BASIC SYNTAX"),
                                                   lines = Seq("--FCN functionName|ID|param1=p1|param2=p2|..."), level = 2),
           ...
           val svwseq : Seq[SVcfWalker] = ssseq.flatMap{ case (mapType,params) => {
             ...
             if(mapType == "tagVariantsExpression"){
               Some( VcfExpressionTag(expr = params("expr"), tagID = params("mapID"), tagDesc = params("desc")) );
                ...
                val vcfCodes : VCFAnnoCodes = VCFAnnoCodes(CT_INFIX = params("tagPrefix"))
                Some( SAddGroupInfoAnno(groupFile = params.get("groupFile"),
                                                ...
                                                GTTag = params("inputGT"),
                                                vcfCodes = vcfCodes,
                                                addAlle = params.isSet("calcAlle"), addCounts = params.isSet("calcCounts"), addFreq = params.isSet("calcFreqs"), addMiss = params.isSet("calcMiss"),
                                                addHetHom = params.isSet("calcHetHom"), addMultiHet = params.isSet("calcMultiHet"),
                                                expr=params.get("expr")
                                                ))
             } else if(mapType == "sampleLists"){
                Some(AddAltSampLists(tagGT = params("inputGT"),
                             outputTagPrefix = params("mapID")+"SAMPLIST_" ,
