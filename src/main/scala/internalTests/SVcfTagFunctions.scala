 ...
 */
  ...
  val vcfTagFcnMap_sideEffecting : Map[String,VcfTagFcnFactorySideEffecting] = Seq[VcfTagFcnFactorySideEffecting](
        ...
        new VcfTagFcnFactorySideEffecting(){
          val mmd =  new VcfTagFcnMetadata(
              id = "gtCountTabulate",synon = Seq(),
              shortDesc = "Calculates a conditional sum across the whole file.",
              desc = "Takes as input a String GENO field gtTag, a variant expression varExpr and a genotype level expression gtExpr. "+
                     "This function will tabulate observed values of gtTag for all cases where both gtExpr and varExpr are TRUE. x should be a categorical or integer variable of some sort with a limited number of possible values. "+
                     "Put another way, this function will count the number of occurrances of each observed value of x for which gtExpr and varExpr are true."+
                     "",
              params = Seq[VcfTagFunctionParam](
                  VcfTagFunctionParam( id = "gtTag", ty = "GENO:String|INFO:String",req=true,dotdot=false ),
                  VcfTagFunctionParam( id = "varExpr", ty = "String",req=true,dotdot=false ),
                  VcfTagFunctionParam( id = "gtExpr", ty = "String",req=true,dotdot=false ),
              )
          );
          ...
          def gen(paramValues : Seq[String], outHeader: SVcfHeader, newTag : String, digits : Option[Int] = None) : VcfTagFcnSideEffecting = {
            new VcfTagFcnSideEffecting(){
              ...
              val gtTag = paramValues.head;
              
              val varexpr = paramValues(1);
              val varparser : SFilterLogicParser[SVcfVariantLine] = internalUtils.VcfTool.sVcfFilterLogicParser;
              val varfilter : SFilterLogic[SVcfVariantLine] = varparser.parseString(varexpr);
              ...
              def run(vc : SVcfVariantLine, vout : SVcfOutputVariantLine){
                ...
                if( gtIdx > -1 ){
                  if(varfilter.keep(vc)){
                    vc.genotypes.genotypeValues(gtIdx).zipWithIndex.foreach{ case (gg,ii) => {
                      if( gtfilter.keep(vc,ii) ){
                        tally(newTag+":"+gg,1)
                      }
                    }}
        ...
        Seq[(String,VcfTagFcnFactorySideEffecting)]( (ff.metadata.id,ff) ) ++ ff.metadata.synon.map{ fss => {
           (fss,ff)
        }}
      }}.toMap
  ...
  val vcfFormatFunMap : Map[String,VcfTagFcnFactory] = Seq[VcfTagFcnFactory](
        ...
        new VcfTagFcnFactory(){
          val mmd =  new VcfTagFcnMetadata(
              ...
              shortDesc = "",
              desc = "Input should be a set of gtFields and optionally a name, with the format tagID:name or just tagID. "+
              ...
              params = Seq[VcfTagFunctionParam](
                  VcfTagFunctionParam( id = "x", ty = "GENO:Int|GENO:String",req=true,dotdot=true )
              )
          );
          def metadata = mmd;
          def gen(paramValues : Seq[String], outHeader: SVcfHeader, newTag : String, digits : Option[Int] = None) : VcfTagFcn = {
            new VcfTagFcn(){
              ...
              def run(vc : SVcfVariantLine, vout : SVcfOutputVariantLine){
                 val gtIxx = ppv.zip(ppn).map{ case (gtTag,outName) => {
                   (gtTag, outName, vc.genotypes.fmt.indexOf(gtTag))
                 }}
                 if( vout.genotypes.genotypeValues.length > 0 ){
                   val outgeno = Array.fill[String]( vout.genotypes.genotypeValues(0).length )("")
                   gtIxx.foreach{ case (gtTag, outName, ix ) => {
                     if( ix > -1) {
                       vc.genotypes.genotypeValues(ix).zipWithIndex.map{ case (vv,sampidx) => {
                         if( vv == "1" ){
                           outgeno(sampidx) = outgeno(sampidx) + "," + outName;
                         }
                       }}
                     }
                   }}
                   outgeno.indices.foreach{ sampidx => {
                       if(outgeno(sampidx).length > 0){
                          outgeno(sampidx) = outgeno(sampidx).tail
                       }
                   }}
                   
                   vout.genotypes.addGenotypeArray( newTag, outgeno)
                 }
                 
                 //vc.genotypes.fmt.indexOf(gtTag)
                  
                /*
                 vout.genotypes.genotypeValues(gtIdx).map{ gg => {
                    gg.split(delim).lift(extractIDX).getOrElse(".");
                  }}
                */
        ...
        new VcfTagFcnFactory(){
          val mmd =  new VcfTagFcnMetadata(
              id = "IF.AB",synon = Seq("SWITCH.EXPR"),
              shortDesc = "",
        ...
        Seq[(String,VcfTagFcnFactory)]( (ff.metadata.id,ff) ) ++ ff.metadata.synon.map{ fss => {
           (fss,ff)
        }}
      }}.toMap
  ...
  val vcfTagFunMap : Map[String,VcfTagFcnFactory] = Seq[VcfTagFcnFactory](
        ...
        new VcfTagFcnFactory(){
          val mmd =  new VcfTagFcnMetadata(
              ...
              desc = "First parameter should be an INFO field, second parameter is a string. Any elements in the INFO field that contain the string will be dropped. "+
                     " Does not do pattern matching, simple text search."+
                     ""+
                     "",
              params = Seq[VcfTagFunctionParam](
                  VcfTagFunctionParam( id = "info", ty = "INFO:String",req=true,dotdot=false ),
        ...
        new VcfTagFcnFactory(){
          val mmd =  new VcfTagFcnMetadata(
              ...
              shortDesc = "",
              desc = "First parameter should be an INFO field, second parameter is a string. Any elements in the INFO field that do not contain the string will be dropped. "+
                     " Does not do pattern matching, simple text search."+
                     ""+
                     "",
              params = Seq[VcfTagFunctionParam](
