...
case class ClinVarMetadata(
  ...
  def getClinVarAssertFromStringV2(s : String, subDelim : String = "|") : ClinVarAssertInfo = {
    ...
    //RCV,SCV,ALLELE_ID,VARIATION_ID,VARIATION_TYPE,CLINICAL_SIGNIFICANCE_ORDERED,REVIEW_STATUS_ORDERED,SUBMITTERS_ORDERED,DATES_ORDERED
    val accessionID = cells(1);
    val date = "";
    val orgID = cells(2);
    val orgType = "";
    val orgName = cells(7)
    ...
    val varID = cells(3);
    val hapID = cells(0);
    val isMultiVarHaplo = 0;
    
    val diseaseLookup = Seq()
    val diseaseMEDGEN = Seq()
    ...
    def walkVCF(vcIter : Iterator[SVcfVariantLine], vcfHeader : SVcfHeader, verbose : Boolean = true) : (Iterator[SVcfVariantLine],SVcfHeader) = {
      ...
      (vcMap(vcIter){ v => {
        ...
        if(assertField != "."){
          ...
          val assertDataFull = getClinVarAssertSeqFromStringV2(assertField)
