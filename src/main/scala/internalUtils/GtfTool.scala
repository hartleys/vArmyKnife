...
object GtfTool {
    ...
    abstract class BedLine {
      def chrom : String;
      def start : Int;
      def end : Int;
      def name : Option[String];
      def score : Option[Int];
      def strand :  Option[Char];
      def tstart :  Option[Int];
      def tend : Option[Int];
      def rgb : Option[Seq[Int]];
      def blockCount : Option[Int];
      def blockSizes : Option[Seq[Int]];
      def blockStarts : Option[Seq[Int]];
      def anno : Seq[String];
      
      def columnList : Seq[String] = {
        var soFar = Seq(chrom,start.toString,end.toString)
        if(name.isEmpty) return soFar;
        soFar = soFar :+ name.get;
        if(score.isEmpty) return soFar;
        soFar = soFar :+ score.get.toString;
        if(strand.isEmpty) return soFar;
        soFar = soFar :+ strand.get.toString;
        
        if(tstart.isEmpty) return soFar;
        soFar = soFar :+ tstart.get.toString;
        if(tend.isEmpty) return soFar;
        soFar = soFar :+ tend.get.toString;
        if(rgb.isEmpty) return soFar;
        soFar = soFar :+ rgb.get.mkString(",");
        
        if(blockCount.isEmpty) return soFar;
        soFar = soFar :+ blockCount.get.toString;
        if(blockSizes.isEmpty) return soFar;
        soFar = soFar :+ blockSizes.get.toString;
        if(blockStarts.isEmpty) return soFar;
        soFar = soFar :+ blockStarts.get.toString;
        
        if(anno.isEmpty) return soFar;
        soFar = soFar ++ anno;
        
        return soFar.map(_.toString);
      }
      
      def getBedString(delim : String) : String = {
        columnList.mkString(delim);
      }
      def getIVs : Seq[commonSeqUtils.GenomicInterval] = {
        if(blockCount.isEmpty || blockCount.get <= 1){
          return Seq( commonSeqUtils.GenomicInterval(chromName = chrom,strand = strand.getOrElse('.'),start = start,end=end) );
        } else {
          return blockSizes.get.zip(blockStarts.get).map{ case (bsize,bstart) => {
            commonSeqUtils.GenomicInterval(chromName = chrom,strand = strand.getOrElse('.'),start = start + bstart,end=start + bstart + bsize)
          }}
        }
      }
    }
    class InputBedLine(line : String) extends InputBedLineCells(line.split("\t")){
      
    }
    
    class InputBedLineCells(cells : Array[String]) extends BedLine {
      //val cells = line.split("\t");
      lazy val line = cells.mkString("\t")
      if(cells.length < 3) error("bad BED line: "+line);
      
      def chrom : String = cells(0);
      lazy val lazyStart = stdUtils.string2int(cells(1));
      lazy val lazyEnd = stdUtils.string2int(cells(2));
      def start : Int = lazyStart;
      def end : Int = lazyEnd;
      
      lazy val lazyName = if(cells.isDefinedAt(3)) Some(cells(3)) else None;
      lazy val lazyScore = if(cells.isDefinedAt(4)) Some(stdUtils.string2int(cells(4))) else None;
      lazy val lazyStrand = if(cells.isDefinedAt(5)) Some(cells(5).charAt(0)) else None;
      lazy val lazytstart = if(cells.isDefinedAt(6)) Some(stdUtils.string2int(cells(6))) else None;
      lazy val lazytend = if(cells.isDefinedAt(7)) Some(stdUtils.string2int(cells(7))) else None;
      lazy val lazyrgb = if(cells.isDefinedAt(8)) {
        if(cells(8) == "."){
          Some(Seq(0,0,0));
        } else {
          val rgbCells = cells(8).split(",");
          if(rgbCells.length != 3) {
            warning("bad RGB code for bed line: "+line,"MALFORMED_BED_RGB",100);
            Some(Seq(0,0,0));
          } else {
            Some(rgbCells.toSeq.map{stdUtils.string2int(_)});
          }
        }
      } else None;
      lazy val lazyblockCount = if(cells.isDefinedAt(9)) Some(stdUtils.string2int(cells(9))) else None;
      lazy val (lazyblockSizes,lazyBlockStarts) = if(cells.isDefinedAt(10)) {
          val bsizecells = cells(10).split(",");
          val bstartcells = cells(11).split(",");
          if(bsizecells.length != blockCount.get || bstartcells.length != blockCount.get) {
            warning("bad block: "+line,"MALFORMED_BED_BLOCK",100);
            (Some(Seq((end - start))),Some(Seq(0)));
          } else {
            (Some(bsizecells.toSeq.map{stdUtils.string2int(_)}),Some(bstartcells.toSeq.map{stdUtils.string2int(_)}))
          }
      } else (None,None);
      
      lazy val lazyAnno = cells.drop(12).toSeq;
      
      def name : Option[String] = lazyName
      def score : Option[Int] = lazyScore
      def strand :  Option[Char] = lazyStrand
      def tstart :  Option[Int] = lazytstart;
      def tend : Option[Int] = lazytend
      def rgb : Option[Seq[Int]]= lazyrgb
      def blockCount : Option[Int] = lazyblockCount
      def blockSizes : Option[Seq[Int]] = lazyblockSizes
      def blockStarts : Option[Seq[Int]] = lazyBlockStarts
      def anno : Seq[String] = lazyAnno
    }
