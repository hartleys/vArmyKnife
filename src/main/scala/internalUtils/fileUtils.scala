...
object fileUtils {
  ...
  class CmdZip extends internalUtils.commandLineUI.CommandLineRunUtil {
     ...
     val parser : internalUtils.commandLineUI.CommandLineArgParser = 
       new internalUtils.commandLineUI.CommandLineArgParser(
          command = "CmdZip", 
          quickSynopsis = "Zips and unzips.", 
          synopsis = "For some reason this is actually faster than linux gzip for compression. I don't know how that's possible, but here we are.", 
          description = "" + internalUtils.commandLineUI.ALPHA_WARNING,
          argList = 
                    new internalUtils.commandLineUI.UnaryArgument( name = "compress",
                                         arg = List("--compress", "-c"), // name of value
                                         argDesc = ""+
                                                   "" // description
                                       ) ::
                    new internalUtils.commandLineUI.UnaryArgument( name = "zip",
                                         arg = List("--zip", "-z"), // name of value
                                         argDesc = ""+
                                                   "" // description
                                       ) ::
                    new internalUtils.commandLineUI.FinalArgument[String](
                                         name = "infile",
 ...
 */
         val indata = if(parser.get[Boolean]("verbose")){
           internalUtils.stdUtils.wrapIteratorWithAdvancedProgressReporter[String](
                  getLinesSmartUnzip(infile = infile, allowStdin = true),
                  internalUtils.stdUtils.AdvancedIteratorProgressReporter_ThreeLevelAuto[String](
                         elementTitle = "lines", lineSec = 60,
                         reportFunction  = ((vc : String, i : Int) => "")
                  )
               );
         } else {
           getLinesSmartUnzip(infile = infile, allowStdin = true)
         }
         val writer = if(parser.get[Boolean]("zip")){
           openZipWriter(outfile);
         } else {
           openGzipWriter(outfile);
         }
         indata.foreach{line => writer.write(line+"\n")}
         writer.close();
       }
     }
  ...
  trait ProgressIterator[A] extends Iterator[A] {
    def hasNext : Boolean;
    def next : A;
    def progress : Double;
    def bytesRemain : Long;
    def numBytes : Long;
  }
  case class FileInputStreamProgressIterator(lines : Iterator[String], fis : FileInputStream) extends ProgressIterator[String] {
    def hasNext : Boolean = lines.hasNext
    def next : String = lines.next
    def progress : Double = {
      fis.getChannel().size().toDouble / fis.getChannel().position().toDouble
    }
    def bytesRemain : Long = fis.getChannel().position()
    def numBytes : Long =  fis.getChannel().size()
  }
  
  def getLinesSmartUnzipWithProgress( infile : String) : ProgressIterator[String] = {
    val fis = new FileInputStream(infile)
    val lines = if(infile.length > 3 && infile.takeRight(3) == ".gz"){
        scala.io.Source.fromInputStream(new GZIPInputStream(new BufferedInputStream(fis)))(DEFAULT_UTF8_CODEC).getLines;
    } else if(infile.length > 4 && infile.takeRight(4) == ".zip"){
        scala.io.Source.fromInputStream(new ZipInputStream(new BufferedInputStream(new FileInputStream(infile))))(DEFAULT_UTF8_CODEC).getLines;
    } else {
      scala.io.Source.fromInputStream(new BufferedInputStream(new FileInputStream(infile)))(DEFAULT_UTF8_CODEC).getLines
    }
    return FileInputStreamProgressIterator(lines,fis);
  }
