...
class qcGtfAnnotationBuilder(gtffile : String, flatgtffile : Option[String], stranded : Boolean, stdCodes : GtfCodes, flatCodes : GtfCodes, targetRegionBed : Option[String] = None){
  ...
  lazy val makeFlatReader : (() => Iterator[FlatGtfLine]) = if(! flatgtffile.isEmpty){
    ...
    val flatlines = prepFlatGtfFile.getFlatGtfLines(gtffile,stranded);
  ...
  lazy val txArray : GenomicArrayOfSets[String] = qcGtfAnnotationBuilder.qcGetTxArray_readGtf(stranded,gtffile, stdCodes).finalizeStepVectors;
  lazy val txSpans : Map[String,(String,Int,Int,Char)] = qcGtfAnnotationBuilder.qcGetTxSpans_readGtf(gtffile, stdCodes);
  lazy val txArrayWithBuffer : GenomicArrayOfSets[String] = qcGtfAnnotationBuilder.qcGetTxArrayWithNearBuffer_readGtf(stranded,gtffile, stdCodes).finalizeStepVectors;
...
object qcGtfAnnotationBuilder {
  ...
  private def qcGetTxArray_readGtf(stranded : Boolean, gtffile : String, codes : GtfCodes) : GenomicArrayOfSets[String] = {
    return buildGenomicArrayOfSets_fromGtf(stranded, gtffile, (gtfLine : GtfLine) => gtfLine.featureType == codes.STD_EXON_TYPE_CODE, (gtfLine : GtfLine) => gtfLine.getAttributeOrDie(codes.STD_TX_ID_ATTRIBUTE_KEY) );
  }
  private def qcGetTxArrayWithNearBuffer_readGtf(stranded : Boolean, gtffile : String, codes : GtfCodes) : GenomicArrayOfSets[String] = {
    val txarray = qcGetTxArray_readGtf(stranded,gtffile,codes);
    val spanMap = qcGetTxSpans_readGtf(gtffile,codes);
    spanMap.foreach{case (txid,(chrom,start,end,strand)) => {
      if(strand == '+'){
        txarray.addSpan(GenomicInterval(chrom,strand,start-2000,start).usingStrandedness(stranded), txid);
        txarray.addSpan(GenomicInterval(chrom,strand,end,     end+500).usingStrandedness(stranded), txid);
      } else {
        txarray.addSpan(GenomicInterval(chrom,strand,start-500,start).usingStrandedness(stranded), txid);
        txarray.addSpan(GenomicInterval(chrom,strand,end,     end+2000).usingStrandedness(stranded), txid);
      }
      //GenomicInterval(chromName : String, strand : Char, start : Int, end : Int)
    }}
    return txarray;
  }
  
  private def qcGetTxSpans_readGtf(gtffile : String, codes : GtfCodes) : Map[String,(String,Int,Int,Char)] = {
    val gtfReader = GtfReader.getGtfReader(gtffile, true, true, "\\s+");
    val out = new scala.collection.mutable.AnyRefMap[String,(String,Int,Int,Char)]( defaultEntry = (x : String) => (("",Int.MaxValue,Int.MinValue,'.')) )
    for(gtfLine <- gtfReader){
      if(gtfLine.featureType == codes.STD_EXON_TYPE_CODE){
        val txid = gtfLine.getAttributeOrDie(codes.STD_TX_ID_ATTRIBUTE_KEY);
        val (oldChrom,oldStart,oldEnd,oldStrand) = out(txid);
        if(oldChrom != gtfLine.chromName && oldChrom != "") error("FATAL ERROR: TX "+txid+" appears on multiple chromosomes!");
        if(oldStrand != gtfLine.strand && oldStrand != '.') error("FATAL ERROR: TX "+txid+" appears on multiple strands!");
        out(txid) = (oldChrom,math.max(oldStart,gtfLine.start-1),math.min(oldEnd,gtfLine.end),oldStrand);
      }
    }
    return out.toMap;
  }
